	...
	bool OnInit()
	{
		...
		if ( args.size() > 1 )
		{
			for( size_t i=1;i<args.size();i++ )
			{
				...
				ScriptWindow *sw = ScriptWindow::CreateNewWindow( !run );
				if ( sw->Load( args[i] ) )
				{
			
					if ( run )
					{
						if ( sw->Run() ) // if run OK, close window
							ScheduleForDestruction( sw );
						else
						{
...
bool ScriptWindow::Run()
{
	...
	Layout();
	wxSafeYield( 0, true );
	bool ok = m_script->Execute( wxPathOnly(m_fileName) );
	m_stopBtn->Hide();
	m_runBtn->Show();
	Layout();
	return ok;
