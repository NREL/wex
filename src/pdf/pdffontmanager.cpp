...
public:
    ...
    wxPdfFontListEntry &operator=(const wxPdfFontListEntry &p) { // DO NOT CHANGE THE ORDER OF THESE STATEMENTS!
    ...
    if (FindFile(fontCollectionFileName, fullFontCollectionFileName)) {
        ...
        if (fileName.IsOk() && fileName.GetExt().Lower().IsSameAs(wxT("ttc"))) {
            ...
            wxLogWarning(wxString(wxT("wxPdfFontManagerBase::RegisterFontCollection: ")) +
                         wxString::Format(_("Font collection file '%s' has not the file extension '.ttc'."),
                                          fontCollectionFileName.c_str()));
        }
    } else {
        wxLogError(wxString(wxT("wxPdfFontManagerBase::RegisterFontCollection: ")) +
                   wxString::Format(_("Font collection file '%s' does not exist or is not readable."),
                                    fontCollectionFileName.c_str()));
    }
    return count;
}
    ...
    if (familyIter == m_fontFamilyMap.end()) {
        ...
        if (FindFile(fontFileName, fullFontFileName)) {
            ...
            wxLogError(wxString(wxT("wxPdfFontManagerBase::RegisterFontCJK: ")) +
                       wxString::Format(_("CJK Font file '%s' for CJK family '%s' does not exist or is not readable."),
                                        fontFileName.c_str(), family.c_str()));
        }
    } else {
...
#endif
                {
                    if (fontType.IsSameAs(wxT("TrueType"))) {
                        ...
                        wxLogError(wxString(wxT("wxPdfFontManagerBase::LoadFontFromXML: ")) +
                                   wxString::Format(_("Unknown font type '%s' in font file '%s'."), fontType.c_str(),
                                                    fontFileName.c_str()));
                    }
                    if (fontData != NULL) {
                        ...
                        if (!fontData->LoadFontMetrics(root)) {
                            wxLogError(wxString(wxT("wxPdfFontManagerBase::LoadFontFromXML: ")) +
                                       wxString::Format(_("Loading of font metrics failed for font file '%s'."),
                                                        fontFileName.c_str()));
                            delete fontData;
                            fontData = NULL;
                        }
