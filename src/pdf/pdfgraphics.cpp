            ...
            case wxPDF_SEG_LINETO: {
                ...
                if (distance >= next) {
                    ...
                    while (currentChar < length && distance >= next) {
                        ...
                        nextAdvance =
                                currentChar < length - 1 ? DoGetStringWidth(voText.Mid(currentChar + 1, 1)) * 0.5 :
...
wxPdfDocument::RegularPolygon(double x0, double y0, double r, int ns, double angle, bool circle, int style,
                              int circleStyle, const wxPdfLineStyle &circleLineStyle,
                              const wxPdfColour &circleFillColour) {
    if (ns < 3) {
        ns = 3;
    }
    if (circle) {
