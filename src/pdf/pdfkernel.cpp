...
wxPdfDocument::SelectFont(const wxString &family, int style, double size, bool setFont) {
    wxString fontFamily = (!family.IsEmpty()) ? family : (m_currentFont != NULL) ? m_currentFont->GetFontFamily()
                                                                                 : wxString();
    bool selected = false;
    ...
    if (regFont.IsValid()) {
        ...
        wxLogError(wxString(wxT("wxPdfDocument::SetFont: ")) +
                   wxString::Format(_("No font registered for font family '%s' with style '%d'."), fontFamily.c_str(),
                                    style));
    }
    return selected;
}
...
wxPdfDocument::SelectFont(const wxPdfFont &font, int style, double size, bool setFont) {
    ...
    if (font.IsValid()) {
        ...
        if (wxPdfFontManager::GetFontManager()->InitializeFontData(font)) {
            wxString fontKey = wxString::Format(wxT("%s[%s]"), font.GetName().Lower().c_str(),
                                                font.GetEncoding().Lower().c_str());
            // Test if font is already selected
            if (m_currentFont != NULL) {
                wxString currentFontKey = wxString::Format(wxT("%s[%s]"),
                                                           m_currentFont->GetOriginalName().Lower().c_str(),
                                                           m_currentFont->GetFont().GetEncoding().Lower().c_str());
        ...
        switch (type) {
            ...
            case wxPDF_GRADIENT_RADIAL: {
                ...
                if (type == wxPDF_GRADIENT_MIDAXIAL) {
                    ...
                    OutAscii(
                            wxString(wxT("/Bounds [")) + wxPdfUtility::Double2String(midpoint, 3) + wxString(wxT("]")));
...
wxPdfDocument::PutFonts() {
    ...
    for (fontIter = m_fonts->begin(); fontIter != m_fonts->end(); fontIter++) {
        ...
        if (type == wxT("core")) {
            ...
            OutAscii(wxString(wxT("/W ")) +
                     font->GetWidthsAsString()); // A description of the widths for the glyphs in the CIDFont
            Out(">>");
            Out("endobj");
    ...
    for (iter = 0; iter < 2; iter++) {
        ...
        for (image = m_images->begin(); image != m_images->end(); image++) {
            ...
            if (currentImage->IsFormObject()) {
                ...
                OutAscii(wxString::Format(wxT("/Length %lu>>"),
                                          (unsigned long) CalculateStreamLength(currentImage->GetDataSize())));
        ...
        if (currentTemplate->GetResources() != NULL) {
            ...
            if (currentTemplate->m_fonts->size() > 0) {
                ...
                for (font = currentTemplate->m_fonts->begin(); font != currentTemplate->m_fonts->end(); font++) {
                    OutAscii(wxString::Format(wxT("/F%d %d 0 R"), font->second->GetIndex(),
                                              font->second->GetObjIndex()));
                }
                Out(">>");
            }
            ...
            if (currentTemplate->m_images->size() > 0 ||
                ...
                for (image = currentTemplate->m_images->begin(); image != currentTemplate->m_images->end(); image++) {
                    wxPdfImage *currentImage = image->second;
                    OutAscii(wxString::Format(wxT("/I%d %d 0 R"), currentImage->GetIndex(),
                                              currentImage->GetObjIndex()));
                }
                wxPdfTemplatesMap::iterator templateIterTmp = currentTemplate->m_templates->begin();
                for (templateIterTmp = currentTemplate->m_templates->begin();
                     templateIterTmp != currentTemplate->m_templates->end(); templateIterTmp++) {
                    wxPdfTemplate *tpl = templateIterTmp->second;
                    OutAscii(
                            m_templatePrefix + wxString::Format(wxT("%d %d 0 R"), tpl->GetIndex(), tpl->GetObjIndex()));
                }
                Out(">>");
            }
            Out(">>");
...
wxPdfDocument::PutResourceDict() {
    ...
    if (!m_spotColours->empty()) {
        ...
        for (spotColour = m_spotColours->begin(); spotColour != m_spotColours->end(); spotColour++) {
            OutAscii(wxString::Format(wxT("/CS%d %d 0 R"), spotColour->second->GetIndex(),
...
wxPdfDocument::PutSpotColours() {
    ...
    for (spotIter = (*m_spotColours).begin(); spotIter != (*m_spotColours).end(); spotIter++) {
        ...
        OutAscii(wxT("/C1 [") +
                 wxPdfUtility::Double2String(wxPdfUtility::ForceRange(spotColour->GetCyan(), 0., 100.) / 100., 4) +
                 wxT(" ") +
...
wxPdfDocument::PutPatterns() {
    ...
    for (patternIter = m_patterns->begin(); patternIter != m_patterns->end(); patternIter++) {
        ...
        OutAscii(wxString::Format(wxT("/Resources << /XObject << /I%d %d 0 R >> >>"), image->GetIndex(),
                                  image->GetObjIndex()));
        ...
        OutAscii(wxString(wxT("/Length ")) +
                 wxString::Format(wxT("%lu"), (unsigned long) CalculateStreamLength(mos.TellO())));
        Out(">>");
        PutStream(mos);
        Out("endobj");
    }
