...
wxPdfFont &
wxPdfFont::operator=(const wxPdfFont &font) { // DO NOT CHANGE THE ORDER OF THESE STATEMENTS!
            ...
            if (ok) {
                if (!((wxPdfFontDataType1 *) m_fontData)->GetEncodingType().IsEmpty()) {
                    ...
                    wxLogDebug(wxString(wxT("wxPdfFont::SetEncoding: ")) +
                               wxString(
                                       _("Setting a user defined encoding is only supported for dynamically loaded Type1 fonts.")));
                }
            } else {
                wxLogDebug(wxString(wxT("wxPdfFont::SetEncoding: ")) +
