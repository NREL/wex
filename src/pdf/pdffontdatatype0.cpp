...
wxString
wxPdfFontDataType0::GetWidthsAsString(bool subset, wxPdfSortedArrayInt *usedGlyphs,
...
wxPdfFontDataType0::GetStringWidth(const wxString &s, const wxPdfEncoding *encoding, bool withKerning) const {
    ...
    for (ch = t.begin(); ch != t.end(); ++ch) {
        wxChar c = *ch;
        if (c >= 0 && c < 128) {
            wxPdfGlyphWidthMap::iterator charIterTmp = (*m_cw).find(c);
            if (charIterTmp != (*m_cw).end()) {
                w += charIterTmp->second;
            } else {
