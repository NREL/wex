...
wxPdfImage::wxPdfImage(wxPdfDocument *document, int index, const wxString &name, wxInputStream &stream,
...
wxPdfImage::ParsePNG(wxInputStream *imageStream) {
    ...
    imageStream->Read(buffer, 4);
    m_parms = wxString::Format(wxT("/DecodeParms <</Predictor 15 /Colors %d /BitsPerComponent %d /Columns %d>>"),
                               (ct == 2 ? 3 : 1), (int) bpc, w);
                ...
                switch (obj->type) {
                    ...
                    case 'P':
                        ...
                        if (!nullPen) {
                            ...
                            data += wxPdfUtility::Double2String(obj->width * m_document->m_k, 2) +
                                    wxString(wxT(" w\n"));
                        }
