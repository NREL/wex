                ...
                if (mainTag == wxT("ttcf")) {
                    ...
                    if (fontIndex < dirCount) {
                        ...
                        wxLogError(wxString(wxT("wxPdfFontParserTrueType::IdentifyFont: ")) +
                                   wxString::Format(_("Font index %d out of range for font file '%s'."), fontIndex,
                                                    fontFileName.c_str()));
                        ok = false;
                    }
                } else {
                    wxLogError(wxString(wxT("wxPdfFontParserTrueType::IdentifyFont: '")) +
                               wxString::Format(_("Font file '%s' not a valid TrueType collection (TTC) file."),
                                                fontFileName.c_str()));
                    ok = false;
                }
            } else {
                ...
                if (ReadTableDirectory()) {
                    if (CheckTables()) {
                        ...
                        if (ok) {
                            ...
                            wxLogError(wxString(wxT("wxPdfFontParserTrueType::LoadFontData: ")) +
                                       wxString::Format(_("Wrong font data type '%s' for font file '%s'."),
                                                        fontData->GetType().c_str(), m_fileName.c_str()));
                        }
                    } else {
                        wxLogError(wxString(wxT("wxPdfFontParserTrueType::LoadFontData: ")) +
        ...
        if (id == 0x00010000 || id == 0x4F54544F || id == 0x74727565) {
            ...
            if (!m_fileName.IsEmpty()) {
                wxLogError(wxString(wxT("wxPdfFontParserTrueType::ReadTableDirectory: '")) +
                           wxString::Format(_("Font file '%s' not a valid TrueType (TTF) or OpenType (OTF) file."),
                                            m_fileName.c_str()));
            }
            ok = false;
        }
