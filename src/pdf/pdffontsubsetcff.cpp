...
bool
wxPdfFontSubsetCff::ReadPrivateDict(wxPdfCffDictionary *privateDict, wxPdfCffIndexArray *localSubrIndex, int offset,
                                    int size) {
    ...
    for (j = 0; ok && (j < m_numFontDicts); j++) {
        ...
        if (ok) {
            ...
            if (ok) {
                ...
                ok = ReadPrivateDict((wxPdfCffDictionary *) m_fdPrivateDict[j],
                                     (wxPdfCffIndexArray *) m_fdLocalSubrIndex[j], offset, size);
                if (ok) {
                    wxMemoryOutputStream buffer;
                    EncodeIntegerMax(0, buffer);
                    EncodeIntegerMax(0, buffer);
...
wxPdfFontSubsetCff::WriteCidPrivateDictAndLocalSub() {
    if (m_isCid) {
        int j;
        for (j = 0; j < m_numSubsetFontDicts; j++) {
            WritePrivateDict(j, (wxPdfCffDictionary *) m_fdDict[m_fdSubsetMap[j]],
                             (wxPdfCffDictionary *) m_fdPrivateDict[m_fdSubsetMap[j]]);
        }
        for (j = 0; j < m_numSubsetFontDicts; j++) {
            WriteLocalSub(j, (wxPdfCffDictionary *) m_fdPrivateDict[m_fdSubsetMap[j]],
                          (wxPdfCffIndexArray *) m_fdLocalSubrIndex[m_fdSubsetMap[j]]);
        }
    } else {
...
wxPdfFontSubsetCff::FindSubrsUsed(int fd, wxPdfCffIndexArray &localSubrIndex,
    ...
    for (j = 0; j < m_usedGlyphs.GetCount(); j++) {
        ...
        if (fdGlyph == fd) {
            ...
            m_decoder->ReadASubr(m_inFont, beginChar, endChar, m_globalBias, localBias, hSubrsUsed, lSubrsUsed,
                                 localSubrIndex);
        }
    }
    ...
    for (k = 0; k < m_lGlobalSubrsUsed.GetCount(); k++) {
        ...
        if (subr < nGlobalSubrs && subr >= 0) {
            ...
            if (m_isCid) {
                ...
                m_decoder->ReadASubr(m_inFont, start, end, m_globalBias, 0, *m_hGlobalSubrsUsed, m_lGlobalSubrsUsed,
                                     dummy);
            } else {
#if 0
