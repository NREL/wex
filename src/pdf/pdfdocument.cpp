...
wxPdfDocument::AddPage(int orientation, wxSize pageSize) {
    if (m_inTemplate) {
        wxLogError(wxString(wxT("wxPdfDocument::AddPage: ")) +
                   wxString::Format(_("Adding pages in templates is impossible. Current template ID is %d."),
                                    m_templateId));
        return;
    }
...
void
wxPdfDocument::RotatedText(double textX, double textY, double rotationX, double rotationY, const wxString &txt,
                           double angle) {
    // Text rotated around its origin
    if (angle == 0) {
...
void
wxPdfDocument::Cell(double w, double h, const wxString &txt, int border, int ln, int align, int fill,
...
void
wxPdfDocument::DoCell(double w, double h, const wxString &txt, int border, int ln, int align, int fill,
                      const wxPdfLink &link) {
    // Output a cell
    double x, y;
    double k = m_k;
    ...
    if (img.Ok()) {
        ...
        if (image == (*m_images).end()) {
            if (tempImage.HasAlpha()) {
                ...
                // Extract the mask
                wxImage mask = tempImage.ConvertToMono(tempImage.GetMaskRed(), tempImage.GetMaskGreen(),
                                                       tempImage.GetMaskBlue());
                // Invert the mask
