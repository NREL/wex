...
pointT *qh_detvnorm(vertexT *vertex, vertexT *vertexA, setT *centers, realT *offsetp) {
    ...
    if (qh VERIFYoutput || qh PRINTstatistics) {
        ...
        if (!unbounded) {
            ...
            trace4((qh
                           ferr, 4014, "qh_detvnorm: points %d %d midpoint dist %2.2g\n",
            ...
            if (angle < 0.0)
                angle -= angle;
            trace4((qh
                           ferr, 4015, "qh_detvnorm: points %d %d angle %2.2g nearzero %d\n",
        ...
        if (simplex != points) {
            FOREACHpoint_i_(points) {
                    if (!qh_setin(simplex, point)) {
                        ...
                        wwadd_(Wridge, dist);
                        trace4((qh
                                       ferr, 4016, "qh_detvnorm: points %d %d Voronoi vertex %d dist %2.2g\n",
                ...
                FOREACHvertex_(neighbor->vertices) {
                        if (vertex->visitid != qh vertex_visit && !vertex->seen) {
                            ...
                            if (count >= qh hull_dim - 1) {  /* e.g., 3 for 3-d Voronoi */
                                ...
                                totridges++;
                                trace4((qh
                                               ferr, 4017, "qh_eachvoronoi: Voronoi ridge of %d vertices between sites %d and %d\n",
...
void qh_markkeep(facetT *facetlist) {
    ...
    trace2((qh
                   ferr, 2006, "qh_markkeep: only keep %d largest and/or %d most merged facets and/or min area %.2g\n",
                           qh
                   KEEParea, qh
                   KEEPmerge, qh
                   KEEPminArea));
...
setT *qh_markvoronoi(facetT *facetlist, setT *facets, boolT printall, boolT *isLowerp, int *numcentersp) {
    ...
    *numcentersp = numcenters;
    trace2((qh
                   ferr, 2007, "qh_markvoronoi: isLower %d numcenters %d\n", isLower, numcenters));
    return vertices;
} /* markvoronoi */
...
void qh_order_vertexneighbors(vertexT *vertex) {
    ...
    trace4((qh
                   ferr, 4018, "qh_order_vertexneighbors: order neighbors of v%d for 3-d\n", vertex->id));
...
void qh_printafacet(FILE *fp, qh_PRINT format, facetT *facet, boolT printall) {
    ...
    switch (format) {
        ...
        case qh_PRINTcoplanars:
            ...
            FOREACHpoint_(facet->coplanarset)qh_fprintf(fp, 9013, " %d", qh_pointid(point));
            qh_fprintf(fp, 9014, "\n");
            break;
        ...
        case qh_PRINTneighbors:
            ...
            FOREACHneighbor_(facet)qh_fprintf(fp, 9024, " %d",
        ...
        case qh_PRINTvertices:
            ...
            FOREACHvertex_(facet->vertices)qh_fprintf(fp, 9031, " %d", qh_pointid(vertex->point));
            qh_fprintf(fp, 9032, "\n");
            break;
        default:
            break;
        ...
        case qh_PRINTgeom:
            ...
            if (qh PREmerge) {
                maximize_(qh
                                  ...
                                  premerge_centrum + qh
                                  DISTround);
            } else if (qh POSTmerge) maximize_(qh
                                                       PRINTcradius, qh
                                                       postmerge_centrum + qh
                                                       DISTround);
            qh PRINTradius = qh PRINTcradius;
            if (qh PRINTspheres + qh PRINTcoplanar) maximize_(qh
                                                                      PRINTradius, qh
                                                                      MAXabs_coord * qh_MINradius);
            if (qh premerge_cos < REALmax / 2) {
                maximize_(qh
                                  PRINTradius, (1 - qh
                                  premerge_cos) * qh MAXabs_coord);
            } else if (!qh PREmerge && qh POSTmerge && qh postmerge_cos < REALmax / 2) {
                maximize_(qh
                                  PRINTradius, (1 - qh
                                  postmerge_cos) * qh MAXabs_coord);
            }
            maximize_(qh
                              PRINTradius, qh
                              MINvisible);
            ...
            if (qh PRINTdim != 4 &&
                ...
                if (qh PRINTcoplanar || qh PRINTcentrums) {
                    ...
                    if (qh PRINTcoplanar && !qh PRINTspheres) {
                        FOREACHvertex_(vertices)qh_printpointvect2(fp, vertex->point, NULL, qh interior_point,
                                                                   qh PRINTradius);
                    }
                    FORALLfacet_(facetlist) {
                            if (!printall && qh_skipfacet(facet))
                            ...
                            if (!qh PRINTcoplanar)
                                continue;
                            FOREACHpoint_(facet->coplanarset)qh_printpointvect2(fp, point, facet->normal, NULL,
                                                                                qh PRINTradius);
                            FOREACHpoint_(facet->outsideset)qh_printpointvect2(fp, point, facet->normal, NULL,
                                                                               qh PRINTradius);
                        }
                    FOREACHfacet_(facets) {
                            if (!printall && qh_skipfacet(facet))
        ...
        case qh_PRINTtriangles:
            ...
            if (format == qh_PRINToff || qh hull_dim == 2)
                ...
                qh_fprintf(fp, 9061, "%d\n%d %d %d\n", num, qh printoutvar
                                                            + numfacets - numsimplicial, numsimplicial + numridges,
                           totneighbors / 2);
            }
            FORALLpoints qh_printpointid(qh fout, NULL, num, point, qh_IDunknown);
            FOREACHpoint_(qh
                                  other_points)qh_printpointid(qh fout, NULL, num, point, qh_IDunknown);
...
void qh_printfacet3geom_points(FILE *fp, setT *points, facetT *facet, realT offset, realT color[3]) {
    ...
    if (offset != 0.0) {
        ...
        FOREACHpoint_(points)qh_setappend(&printpoints, qh_projectpoint(point, facet, -offset));
    } else
        printpoints = points;
...
void qh_printfacet3vertex(FILE *fp, facetT *facet, qh_PRINT format) {
    ...
    FOREACHvertex_(vertices)qh_fprintf(fp, 9112, "%d ", qh_pointid(vertex->point));
    qh_fprintf(fp, 9113, "\n");
    qh_settempfree(&vertices);
} /* printfacet3vertex */
...
void qh_printfacetNvertex_nonsimplicial(FILE *fp, facetT *facet, int id, qh_PRINT format) {
    ...
    FOREACHridge_(facet->ridges) {
            ...
            if ((ridge->top == facet) ^ qh_ORIENTclock) {
                FOREACHvertex_(ridge->vertices)qh_fprintf(fp, 9126, "%d ", qh_pointid(vertex->point));
            } else {
                FOREACHvertexreverse12_(ridge->vertices)qh_fprintf(fp, 9127, "%d ", qh_pointid(vertex->point));
            }
            qh_fprintf(fp, 9128, "\n");
        }
...
void qh_printfacetNvertex_simplicial(FILE *fp, facetT *facet, qh_PRINT format) {
    ...
    if ((facet->toporient ^ qh_ORIENTclock)
        ...
        FOREACHvertex_(facet->vertices)qh_fprintf(fp, 9130, "%d ", qh_pointid(vertex->point));
    } else {
        FOREACHvertexreverse12_(facet->vertices)qh_fprintf(fp, 9131, "%d ", qh_pointid(vertex->point));
    }
    qh_fprintf(fp, 9132, "\n");
...
void qh_printfacetridges(FILE *fp, facetT *facet) {
    ...
    if (facet->visible && qh NEWfacets) {
        ...
        FOREACHridge_(facet->ridges)qh_fprintf(fp, 9180, " r%d", ridge->id);
        qh_fprintf(fp, 9181, "\n");
    } else {
        ...
        if (numridges != qh_setsize(facet->ridges)) {
            ...
            FOREACHridge_(facet->ridges)qh_fprintf(fp, 9184, " r%d", ridge->id);
            qh_fprintf(fp, 9185, "\n");
        }
        FOREACHridge_(facet->ridges) {
                if (!ridge->seen)
...
void qh_printvertices(FILE *fp, const char *string, setT *vertices) {
    ...
    FOREACHvertex_(vertices)qh_fprintf(fp, 9246, " p%d(v%d)", qh_pointid(vertex->point), vertex->id);
    qh_fprintf(fp, 9247, "\n");
} /* printvertices */
...
void qh_printvneighbors(FILE *fp, facetT *facetlist, setT *facets, boolT printall) {
    ...
    FORALLfacet_(facetlist) {
            FOREACHpoint_(facet->coplanarset)qh_point_add(coplanar_points, point, facet);
        }
    FOREACHfacet_(facets) {
            FOREACHpoint_(facet->coplanarset)qh_point_add(coplanar_points, point, facet);
        }
            ...
            if (vertex) {
                ...
                if (qh hull_dim == 3)
                    ...
                    qsort(SETaddr_(vertex->neighbors, facetT), (size_t) numneighbors,
                          sizeof(facetT *), qh_compare_facetvisit);
                FOREACHneighbor_(vertex)qh_fprintf(fp, 9250, " %d",
                                                   neighbor->visitid ? neighbor->visitid - 1 : 0 - neighbor->id);
                qh_fprintf(fp, 9251, "\n");
...
void qh_printvridge(FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT unbounded) {
    ...
    FOREACHfacet_(centers)qh_fprintf(fp, 9276, " %d", facet->visitid);
    qh_fprintf(fp, 9277, "\n");
} /* printvridge */
...
Type 'qhull' for a short list of options.\n");
    ...
    qh temp_malloc = NULL;
    trace1((qh
                   ferr, 1008, "qh_readpoints: read in %d %d-dimensional points\n",
                           numinput, diminput));
    return (points);
} /* readpoints */
