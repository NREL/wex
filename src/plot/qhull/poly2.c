...
void qh_check_bestdist(void) {
    ...
    trace1((qh
                   ferr, 1020, "qh_check_bestdist: check points below nearest facet.  Facet_list f%d\n",
                           qh
                   facet_list->id));
    ...
    trace1((qh
                   ferr, 1021, "qh_check_bestdist: check that all points are within %2.2g of best facet\n", maxoutside));
...
options 'Qci Tv' to verify all points.\n", notverified);
    ...
    trace0((qh
                   ferr, 20, "qh_check_bestdist: max distance outside %2.2g\n", maxdist));
} /* check_bestdist */
...
void qh_check_dupridge(facetT *facet1, realT dist1, facetT *facet2, realT dist2) {
    ...
    FOREACHvertex_(
            facet1->vertices) {     /* The duplicate ridge is between facet1 and facet2, so either facet can be tested */
    ...
    trace0((qh
                   ferr, 16, "qh_check_dupridge: duplicate ridge between f%d and f%d due to nearly-coincident vertices (%2.2g), dist %2.2g, reverse dist %2.2g, ratio %2.2g while processing p%d\n",
                           facet1->id, facet2->id, minvertex, dist1, dist2, ratio, qh
                   furthest_id));
    if (ratio > qh_WIDEduplicate) {
        qh_fprintf(qh ferr, 6271,
...
void qh_check_maxout(void) {
    ...
    trace1((qh
                   ferr, 1022, "qh_check_maxout: check and update maxoutside for each facet.\n"));
    ...
    if (qh VERTEXneighbors
        ...
        trace1((qh
                       ferr, 1023, "qh_check_maxout: determine actual maxoutside and minvertex\n"));
    ...
    wmax_(Wmaxoutside, qh
            max_outside);
    qh max_outside = maxoutside;
    qh_nearcoplanar(/*qh.facet_list*/);
    qh maxoutdone = True;
    trace1((qh
                   ferr, 1024, "qh_check_maxout: maxoutside %2.2g, min_vertex %2.2g, outside of not good %d\n",
                           maxoutside, qh
                   min_vertex, notgood));
} /* check_maxout */
#else /* qh_NOmerge */
...
void qh_check_points(void) {
    ...
    /* one more qh.DISTround for check computation */
    trace1((qh
                   ferr, 1025, "qh_check_points: check all points below %2.2g of all facet planes\n",
                           maxoutside));
...
all %sfacets.  Will make %2.0f distance computations.\n",
        ...
        trace0((qh
                       ferr, 21, "qh_check_points: max distance outside %2.2g\n", maxdist));
    }
} /* check_points */
...
void qh_checkconvex(facetT *facetlist, int fault) {
    ...
    trace1((qh
                   ferr, 1026, "qh_checkconvex: check all ridges are convex\n"));
    if (!qh RERUN) {
                ...
                FOREACHneighbor_(facet) {
                        ...
                        if (dist > -qh DISTround) {
                            ...
                            if (dist > qh DISTround) {
                                ...
                                qh_precision("coplanar ridge");
                                trace0((qh
                                               ferr, 22, "qhull precision error: f%d may be coplanar to f%d, since p%d(v%d) is within %6.4g during p%d\n",
                                                       facet->id, neighbor->id, qh_pointid(
                                                       vertex->point), vertex->id, dist, qh
                                               furthest_id));
                            }
                        }
                    }
...
void qh_checkpolygon(facetT *facetlist) {
    ...
    trace1((qh
                   ferr, 1027, "qh_checkpolygon: check all facets from f%d\n", facetlist->id));
...
void qh_clearcenters(qh_CENTER type) {
    ...
    trace2((qh
                   ferr, 2043, "qh_clearcenters: switched to center type %d\n", type));
} /* clearcenters */
...
void qh_createsimplex(setT *vertices) {
    ...
    trace1((qh
                   ferr, 1028, "qh_createsimplex: created simplex\n"));
} /* createsimplex */
...
facetT *qh_findbestfacet(pointT *point, boolT bestoutside,
    ...
    trace3((qh
                   ferr, 3014, "qh_findbestfacet: f%d dist %2.2g isoutside %d totpart %d\n",
...
facetT *qh_findbestlower(facetT *upperfacet, pointT *point, realT *bestdistp, int *numpart) {
    ...
    if (!bestfacet) {
        ...
        zmax_(Zbestloweralln, qh
                num_facets);
        /* [dec'15] Previously reported as QH6228 */
        trace3((qh
                       ferr, 3025, "qh_findbestlower: all neighbors of facet %d are flipped or upper Delaunay.  Search all facets\n",
                               upperfacet->id));
        /* rarely called */
        bestfacet = qh_findfacet_all(point, &bestdist, &isoutside, numpart);
    }
    *bestdistp = bestdist;
    trace3((qh
                   ferr, 3015, "qh_findbestlower: f%d dist %2.2g for f%d p%d\n",
...
facetT *qh_findfacet_all(pointT *point, realT *bestdist, boolT *isoutside,
    ...
    *numpart = totpart;
    trace3((qh
                   ferr, 3016, "qh_findfacet_all: f%d dist %2.2g isoutside %d totpart %d\n",
...
int qh_findgood(facetT *facetlist, int goodhorizon) {
    ...
    if (qh GOODthreshold && (numgood || goodhorizon || qh GOODclosest)) {
        ...
        if (!numgood && (!goodhorizon || qh GOODclosest)) {
            ...
            if (bestfacet && bestfacet != qh GOODclosest) {
                ...
                numgood++;
                trace2((qh
                               ferr, 2044, "qh_findgood: f%d is closest(%2.2g) to thresholds\n",
    ...
    trace2((qh
                   ferr, 2045, "qh_findgood: found %d good facets with %d good horizon\n",
                           numgood, goodhorizon));
...
void qh_findgood_all(facetT *facetlist) {
    ...
    if (qh SPLITthresholds) {
        ...
        if (!numgood && bestfacet) {
            ...
            numgood++;
            trace0((qh
                           ferr, 23, "qh_findgood_all: f%d is closest(%2.2g) to thresholds\n",
                                   bestfacet->id, bestangle));
            return;
        }
    }
    qh num_good = numgood;
    trace0((qh
                   ferr, 24, "qh_findgood_all: %d good facets remain out of %d facets\n",
                           numgood, startgood));
} /* findgood_all */
...
void qh_furthestnext(void /* qh.facet_list */) {
    ...
    if (bestfacet) {
        ...
        trace1((qh
                       ferr, 1029, "qh_furthestnext: made f%d next facet(dist %.2g)\n",
...
void qh_furthestout(facetT *facet) {
    ...
    trace3((qh
                   ferr, 3017, "qh_furthestout: p%d is furthest outside point of f%d\n",
...
good point(QGn or QG-n), or a good vertex with 'QJ' or 'Q0' (QVn).\n");
    ...
    qh_settempfree(&maxpoints);
    trace1((qh
                   ferr, 1030, "qh_initbuild: initial hull created and points partitioned\n"));
} /* initbuild */
...
void qh_initialhull(setT *vertices) {
    ...
    FORALLfacets {
        if (!qh_checkflipped(facet, NULL, qh_ALL)) {/* due to axis-parallel facet */
            trace1((qh
                           ferr, 1031, "qh_initialhull: initial orientation incorrect.  Correct all facets\n"));
            facet->flipped = False;
            FORALLfacets {
...
setT *qh_initialvertices(int dim, setT *maxpoints, pointT *points, int numpoints) {
    ...
    if (qh ALLpoints)
        ...
        qh_maxsimplex(dim, maxpoints, points, numpoints, &simplex);
    FOREACHpoint_(simplex)qh_setaddnth(&vertices, 0, qh_newvertex(point)); /* descending order */
...
vertexT *qh_makenewfacets(pointT *point /*visible_list*/) {
    ...
    FORALLvisible_facets {
        ...
        if (!qh ONLYgood) {
            ...
            else zinc_(Zinsidevisible);
            SETfirst_(visible->neighbors) = NULL;
        }
    }
    trace1((qh
                   ferr, 1032, "qh_makenewfacets: created %d new facets from point p%d to horizon\n",
...
void qh_matchduplicates(facetT *atfacet, int atskip, int hashsize, int *hashcount) {
    ...
    trace2((qh
                   ferr, 2046, "qh_matchduplicates: find duplicate matches for f%d skip %d hash %d hashcount %d\n",
    ...
    for (makematch = 0; makematch < 2; makematch++) {
        ...
        for (newfacet = atfacet, newskip = atskip; newfacet; newfacet = nextfacet, newskip = nextskip) {
            ...
            for (scan = hash; (facet = SETelemt_(qh
                                                         hash_table, scan, facetT));
                 scan = (++scan >= hashsize ? 0 : scan)) {
                    ...
                    if (SETelemt_(facet->neighbors, skip, facetT) != qh_DUPLICATEridge) {
                        ...
                        if (SETelemt_(newfacet->neighbors, newskip, facetT) == qh_DUPLICATEridge) {
                            ...
                            *hashcount -= 2; /* removed two unmatched facets */
                            trace4((qh
                                           ferr, 4059, "qh_matchduplicates: duplicate f%d skip %d matched with new f%d skip %d merge\n",
                        ...
                        trace3((qh
                                       ferr, 3018, "qh_matchduplicates: duplicate f%d skip %d new f%d skip %d at dist %2.2g, max is now f%d f%d\n",
        ...
        if (!makematch) {
            ...
            *hashcount -= 2; /* removed two unmatched facets */
            zzinc_(Zmultiridge);
            trace0((qh
                           ferr, 25, "qh_matchduplicates: duplicate f%d skip %d matched with new f%d skip %d keep\n",
...
vertexT *qh_nearvertex(facetT *facet, pointT *point, realT *bestdistp) {
    ...
    if (facet->tricoplanar) {
        ...
        FOREACHneighbor_(apex) {
                if (neighbor->center == center) {
                    FOREACHvertex_(neighbor->vertices)qh_setappend(&vertices, vertex);
                }
            }
    } else
    ...
    *bestdistp = sqrt(bestdist);
    ...
    trace3((qh
                   ferr, 3019, "qh_nearvertex: v%d dist %2.2g for f%d p%d\n",
...
vertexT *qh_newvertex(pointT *point) {
    ...
    vertex->point = point;
    trace4((qh
                   ferr, 4060, "qh_newvertex: vertex p%d(v%d) created\n", qh_pointid(vertex->point),
                           vertex->id));
    return (vertex);
} /* newvertex */
...
void qh_outcoplanar(void /* facet_list */) {
    ...
    trace1((qh
                   ferr, 1033, "qh_outcoplanar: move outsideset to coplanarset for qh NARROWhull\n"));
    FORALLfacets {
        FOREACHpoint_(facet->outsideset) {
                qh num_outside--;
...
void qh_prependfacet(facetT *facet, facetT **facetlist) {
    ...
    trace4((qh
                   ferr, 4061, "qh_prependfacet: prepend f%d before f%d\n",
...
void qh_printhashtable(FILE *fp) {
    ...
    FOREACHfacet_i_(qh
            ...
            if (facet) {
                ...
                FOREACHvertex_(facet->vertices)qh_fprintf(fp, 9284, "v%d ", vertex->id);
                qh_fprintf(fp, 9285, "\n neighbors:");
                FOREACHneighbor_i_(facet) {
...
void qh_printlists(void) {
    ...
    qh_fprintf(qh ferr, 8111, "\n  new facets %d visible facets %d next facet for qh_addpoint %d\n  vertices(new %d):",
               getid_(qh
                              newfacet_list), getid_(qh
                                                             visible_list), getid_(qh
                                                                                           facet_next),
               getid_(qh
                              newvertex_list));
    count = 0;
...
void qh_resetlists(boolT stats, boolT resetVisible /*qh.newvertex_list newfacet_list visible_list*/) {
    ...
    FORALLvertex_(qh
                          newvertex_list)vertex->newlist = False;
...
void qh_triangulate(void /*qh.facet_list*/) {
    ...
    if (qh hasTriangulation)
        return;
    trace1((qh
                   ferr, 1034, "qh_triangulate: triangulate non-simplicial facets\n"));
    if (qh hull_dim == 2)
        return;
    ...
    trace2((qh
                   ferr, 2047, "qh_triangulate: delete null facets from f%d -- apex same as second vertex\n", getid_(
                           new_facet_list)));
    ...
    trace2((qh
                   ferr, 2048, "qh_triangulate: delete %d or more mirror facets -- same vertices and neighbors\n", qh_setsize(
                           qh
                   degen_mergeset)));
    ...
    trace2((qh
                   ferr, 2049, "qh_triangulate: update neighbor lists for vertices from v%d\n", getid_(
                           new_vertex_list)));
    ...
    trace2((qh
                   ferr, 2050, "qh_triangulate: identify degenerate tricoplanar facets from f%d\n", getid_(
                           new_facet_list)));
    trace2((qh
                   ferr, 2051, "qh_triangulate: and replace facet->f.triowner with tricoplanar facets that own center, normal, etc.\n"));
    ...
    trace2((qh
                   ferr, 2052, "qh_triangulate: delete visible facets -- non-simplicial, null, and mirrored facets\n"));
    owner = NULL;
    visible = NULL;
        ...
        if (facet->visible) {
            if (facet->tricoplanar) { /* a null or mirrored facet */
                ...
                if (visible && !owner) {
                    ...
                    trace2((qh
                                   ferr, 2053, "qh_triangulate: all tricoplanar facets degenerate for non-simplicial facet f%d\n",
                                           visible->id));
                    qh_delfacet(visible);
                    qh num_visible--;
                }
    ...
    if (visible && !owner) {
        trace2((qh
                       ferr, 2054, "qh_triangulate: all tricoplanar facets degenerate for last non-simplicial facet f%d\n",
                               visible->id));
        qh_delfacet(visible);
        qh num_visible--;
    }
...
void qh_triangulate_facet(facetT *facetA, vertexT **first_vertex) {
    ...
    trace3((qh
                   ferr, 3020, "qh_triangulate_facet: triangulate facet f%d\n", facetA->id));
    ...
    FORALLnew_facets {
        ...
        if (qh TRInormals) { /* 'Q11' triangulate duplicates ->normal and ->center */
            ...
            if (facetA->normal) {
                ...
                memcpy((char *) newfacet->normal, facetA->normal, qh
                        normal_size);
            }
            if (qh CENTERtype == qh_AScentrum)
                ...
                memcpy((char *) newfacet->center, facetA->center, qh
                        center_size);
            }
        } else {
...
void qh_triangulate_link(facetT *oldfacetA, facetT *facetA, facetT *oldfacetB, facetT *facetB) {
    ...
    trace3((qh
                   ferr, 3021, "qh_triangulate_link: relink old facets f%d and f%d between neighbors f%d and f%d\n",
...
void qh_triangulate_mirror(facetT *facetA, facetT *facetB) {
    ...
    trace3((qh
                   ferr, 3022, "qh_triangulate_mirror: delete mirrored facets f%d and f%d\n",
                           facetA->id, facetB->id));
...
void qh_triangulate_null(facetT *facetA) {
    ...
    trace3((qh
                   ferr, 3023, "qh_triangulate_null: delete null facet f%d\n", facetA->id));
...
void qh_vertexneighbors(void /*qh.facet_list*/) {
    ...
    if (qh VERTEXneighbors)
        return;
    trace1((qh
                   ferr, 1035, "qh_vertexneighbors: determining neighboring facets for each vertex\n"));
    qh vertex_visit++;
    FORALLfacets {
        if (facet->visible)
            continue;
