...
void qh_checkflags(char *command, char *hiddenflags) {
    ...
    if (*hiddenflags != ' ' || hiddenflags[strlen(hiddenflags) - 1] != ' ') {
        qh_fprintf(qh ferr, 6026, "qhull error (qh_checkflags): hiddenflags must start and end with a space: \"%s\"",
                   hiddenflags);
        qh_errexit(qh_ERRinput, NULL, NULL);
    }
    if (strpbrk(hiddenflags, ",\n\r\t")) {
        qh_fprintf(qh ferr, 6027, "qhull error (qh_checkflags): hiddenflags contains commas, newlines, or tabs: \"%s\"",
                   hiddenflags);
        qh_errexit(qh_ERRinput, NULL, NULL);
    }
...
void qh_freebuffers(void) {
    ...
    trace5((qh
                   ferr, 5001, "qh_freebuffers: freeing up global memory buffers\n"));
    ...
    trace5((qh
                   ferr, 5002, "qh_freebuffers: finished\n"));
} /* freebuffers */
...
void qh_freebuild(boolT allmem) {
    ...
    trace1((qh
                   ferr, 1005, "qh_freebuild: free memory from qh_inithull and qh_buildhull\n"));
    if (qh del_vertices)
...
void qh_freeqhull2(boolT allmem) {
    ...
    trace1((qh
                   ferr, 1006, "qh_freeqhull: free global memory\n"));
    qh_freebuild(allmem);
    qh_freebuffers();
    qh_freestatistics();
#if qh_QHpointer
...
void qh_initflags(char *command) {
    ...
    if (command <= &qh qhull_command[0] || command > &qh qhull_command[0] + sizeof(qh qhull_command)) {
        if (command != &qh qhull_command[0]) {
            *qh qhull_command = '\0';
            strncat(qh
                            qhull_command, command, sizeof(qh
                            qhull_command)-strlen(qh qhull_command)-1);
        }
        while (*s && !isspace(*s))  /* skip program name */
            s++;
    }
        ...
        switch (*s++) {
            ...
            case 'H':
                ...
                if (start < t) {
                    ...
                    strncpy(qh
                                    feasible_string, start, (size_t) (t - start));
                                ...
                                if ((r = qh_strtod(s, &s)) == 0.0) {
                                    ...
                                    while (*t && !isspace(*t)) {
                                        if (toupper(*t++) == 'B'
                                            ...
                                            qh PROJECTinput++;
                                            trace2((qh
                                                           ferr, 2004, "qh_initflags: project dimension %d\n", k));
...
void qh_initqhull_globals(coordT *points, int numpoints, int dim, boolT ismalloc) {
    ...
    trace0((qh
                   ferr, 13, "qh_initqhull_globals: for %s | %s\n", qh
                   rbox_command,
                           qh
                   qhull_command));
    ...
    trace2((qh
                   ferr, 2005, "qh_initqhull_globals: initialize globals.  dim %d numpoints %d malloc? %d projected %d to hull_dim %d\n",
                           dim, numpoints, ismalloc, qh
                   PROJECTinput, qh
                   hull_dim));
...
void qh_initqhull_outputflags(void) {
    ...
    trace3((qh
                   ferr, 3024, "qh_initqhull_outputflags: %s\n", qh
                   qhull_command));
...
*/
void qh_lib_check(int qhullLibraryType, int qhTsize, int vertexTsize, int ridgeTsize, int facetTsize, int setTsize,
                  int qhmemTsize) {
    boolT iserror = False;
    ...
    if (iserror) {
        if (qh_QHpointer) {
            qh_fprintf_stderr(6255,
                              "qh_lib_check: Cannot continue.  Library '%s' uses a dynamic qhT via qh_QHpointer (e.g., qhull_p.so)\n",
                              qh_version2);
        } else {
            qh_fprintf_stderr(6256,
                              "qh_lib_check: Cannot continue.  Library '%s' uses a static qhT (e.g., libqhull.so)\n",
                              qh_version2);
        }
...
void qh_option(const char *option, int *i, realT *r) {
    ...
    strncat(qh
                    qhull_options, buf, (size_t) maxlen);
} /* option */
