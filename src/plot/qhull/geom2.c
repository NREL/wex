...
realT qh_determinant(realT **rows, int dim, boolT *nearzero) {
    ...
    *nearzero = False;
    if (dim < 2) {
        ...
        if (fabs_(det) < 10 *
                         qh NEARzero[2])  /* what should this be?  det 5.5e-12 was flat for qh_maxsimplex of qdelaunay 0,0 27,27 -36,36 -9,63  */
            *nearzero = True;
    } else {
...
realT qh_detjoggle(pointT *points, int numpoints, int dimension) {
    ...
    trace2((qh
                   ferr, 2001, "qh_detjoggle: joggle=%2.2g maxwidth=%2.2g\n", joggle, maxwidth));
    return joggle;
} /* detjoggle */
...
void qh_detroundoff(void) {
    ...
    if (qh JOGGLEmax < REALmax / 2 && (qh KEEPcoplanar || qh KEEPinside)) {
        ...
        maximize_(qh
                          NEARinside, maxdist);  /* must agree with qh_nearcoplanar() */
    }
    if (qh KEEPnearinside)
        qh_option("_near-inside", NULL, &qh NEARinside);
    if (qh JOGGLEmax < qh DISTround) {
        qh_fprintf(qh ferr, 6006,
    ...
    if (!qh APPROXhull) {             /* user may specify qh MINoutside */
        ...
        if (qh premerge_cos < REALmax / 2) maximize_(qh
                                                             MINoutside, (1 - qh
...
realT qh_detsimplex(pointT *apex, setT *points, int dim, boolT *nearzero) {
    ...
    trace2((qh
                   ferr, 2002, "qh_detsimplex: det=%2.2g for point p%d, dim %d, nearzero? %d\n",
...
realT qh_distround(int dimension, realT maxabs, realT maxsumabs) {
    ...
    trace4((qh
                   ferr, 4008, "qh_distround: %2.2g maxabs %2.2g maxsumabs %2.2g maxdistsum %2.2g\n",
...
realT qh_facetarea(facetT *facet) {
    ...
    if (facet->simplicial) {
        ...
        FOREACHridge_(facet->ridges)area += qh_facetarea_simplex(qh hull_dim, centrum, ridge->vertices,
                                                                 NULL, (boolT) (ridge->top == facet), facet->normal,
                                                                 &facet->offset);
        if (qh CENTERtype != qh_AScentrum)
    ...
    trace4((qh
                   ferr, 4009, "qh_facetarea: f%d area %2.2g\n", facet->id, area));
    return area;
} /* facetarea */
...
realT qh_facetarea_simplex(int dim, coordT *apex, setT *vertices,
    ...
    area *= qh AREAfactor;
    trace4((qh
                   ferr, 4010, "qh_facetarea_simplex: area=%2.2g for point p%d, toporient %d, nearzero? %d\n",
...
facetT *qh_findgooddist(pointT *point, facetT *facetA, realT *distp,
    ...
    *facetlist = facetA;
    ...
    if (bestfacet) {
        *distp = bestdist;
        trace2((qh
                       ferr, 2003, "qh_findgooddist: p%d is %2.2g above good facet f%d\n",
                               qh_pointid(point), bestdist, bestfacet->id));
        return bestfacet;
    }
    trace4((qh
                   ferr, 4011, "qh_findgooddist: no good facet for p%d above f%d\n",
...
void qh_getarea(facetT *facetlist) {
    ...
    if (qh REPORTfreq)
        qh_fprintf(qh ferr, 8020, "computing area of each facet and volume of the convex hull\n");
                        ...
                        ferr, 1001, "qh_getarea: computing volume and area for each facet\n"));
...
void qh_joggleinput(void) {
    ...
    if (qh build_cnt > 1 && qh JOGGLEmax > fmax_(qh
                                                         MAXwidth / 4, 0.1)) {
        qh_fprintf(qh ferr, 6010,
    ...
    trace0((qh
                   ferr, 6, "qh_joggleinput: joggle input by %2.2g with seed %d\n",
                           qh
                   JOGGLEmax, seed));
...
setT *qh_maxmin(pointT *points, int numpoints, int dimension) {
    ...
    if (REALmin < REALepsilon && REALmin < REALmax && REALmin > -REALmax
        && REALmax > 0.0 && -REALmax < 0.0); /* all ok */
    else {
...
realT qh_maxouter(void) {
    ...
    dist += qh DISTround;
    trace4((qh
                   ferr, 4012, "qh_maxouter: max distance from facet to outer plane is %2.2g max_outside is %2.2g\n", dist, qh
                   max_outside));
    return dist;
} /* maxouter */
...
void qh_maxsimplex(int dim, setT *maxpoints, pointT *points, int numpoints, setT **simplex) {
    ...
    for (k = sizinit; k < dim + 1; k++) {
        ...
        if (!maxpoint || maxnearzero) {
            ...
            if (!maxpoint) {
                trace0((qh
                               ferr, 7, "qh_maxsimplex: searching all points for %d-th initial vertex.\n", k + 1));
            } else {
                trace0((qh
                               ferr, 8, "qh_maxsimplex: searching all points for %d-th initial vertex, better than p%d det %2.2g\n",
        ...
        trace1((qh
                       ferr, 1002, "qh_maxsimplex: selected point p%d for %d`th initial vertex, det=%2.2g\n",
...
void qh_printpoints(FILE *fp, const char *string, setT *points) {
    ...
    if (string) {
        ...
        FOREACHpoint_(points)qh_fprintf(fp, 9005, " p%d", qh_pointid(point));
        qh_fprintf(fp, 9006, "\n");
    } else {
        FOREACHpoint_(points)qh_fprintf(fp, 9007, " %d", qh_pointid(point));
        qh_fprintf(fp, 9008, "\n");
    }
} /* printpoints */
...
void qh_projectinput(void) {
    ...
    trace1((qh
                   ferr, 1003, "qh_projectinput: updating lower and upper_bound\n"));
    ...
    if (qh DELAUNAY && qh ATinfinity) {
        ...
        *(coord++) = maxboloid * 1.1;
        qh num_points++;
        trace0((qh
                       ferr, 9, "qh_projectinput: projected points to paraboloid for Delaunay\n"));
...
void qh_projectpoints(signed char *project, int n, realT *points,
    ...
    trace1((qh
                   ferr, 1004, "qh_projectpoints: projected %d points from dim %d to dim %d\n",
...
void qh_scalelast(coordT *points, int numpoints, int dim, coordT low,
    ...
    trace4((qh
                   ferr, 4013, "qh_scalelast: scale last coordinate from [%2.2g, %2.2g] to [0,%2.2g]\n",
                           low, high, newhigh));
    qh last_low = low;
    qh last_high = high;
...
void qh_scalepoints(pointT *points, int numpoints, int dim,
    ...
    for (k = 0; k < dim; k++) {
        ...
        trace0((qh
                       ferr, 10, "qh_scalepoints: scaled %d'th coordinate [%2.2g, %2.2g] to [%.2g, %.2g] for %d points by %2.2g and shifted %2.2g\n",
...
void qh_setdelaunay(int dim, int count, pointT *points) {
    ...
    trace0((qh
                   ferr, 11, "qh_setdelaunay: project %d points to paraboloid for Delaunay triangulation\n", count));
    coordp = points;
    for (i = 0; i < count; i++) {
        coord = *coordp++;
        paraboloid = coord * coord;
...
coordT *qh_sethalfspace_all(int dim, int count, coordT *halfspaces, pointT *feasible) {
    ...
    trace0((qh
                   ferr, 12, "qh_sethalfspace_all: compute dual for halfspace intersection\n"));
    newdim = dim - 1;
    ...
    if (infinite) {
        ...
        if (qh IStracing >= 3) {
            ...
            if (qh IStracing >= 5) {
                ...
                FOREACHpoint_(simplex)qh_fprintf(qh ferr, 8034, "p%d dist %.2g, ", qh_pointid(point),
