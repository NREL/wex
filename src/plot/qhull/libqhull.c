...
void qh_qhull(void) {
    ...
    if (!qh STOPpoint && !qh STOPcone) {
        ...
        if (qh ZEROall_ok && !qh TESTvneighbors && !qh WAScoplanar) {
            trace2((qh
                           ferr, 2055, "qh_qhull: all facets are clearly convex and no coplanar points.  Post-merging and check of maxout not needed.\n"));
            qh DOcheckmax = False;
        } else {
    ...
    trace1((qh
                   ferr, 1036, "Qhull: algorithm completed\n"));
} /* qhull */
...
boolT qh_addpoint(pointT *furthest, facetT *facet, boolT checkdist) {
    ...
    trace2((qh
                   ferr, 2056, "qh_addpoint: added p%d new facets %d new balance %2.2g point balance %2.2g\n",
...
void qh_buildhull(void) {
    ...
    trace1((qh
                   ferr, 1037, "qh_buildhull: start build hull\n"));
    FORALLfacets {
    ...
    FORALLvertices {
        ...
        if ((qh STOPpoint > 0 && id == qh STOPpoint - 1) ||
            ...
            trace1((qh
                           ferr, 1038, "qh_buildhull: stop point or cone P%d in initial hull\n", id));
            return;
        }
    }
    ...
    trace1((qh
                   ferr, 1039, "qh_buildhull: completed the hull construction\n"));
} /* buildhull */
...
void qh_findhorizon(pointT *point, facetT *facet, int *goodvisible, int *goodhorizon) {
    ...
    trace1((qh
                   ferr, 1040, "qh_findhorizon: find horizon for point p%d facet f%d\n", qh_pointid(point), facet->id));
                        ...
                        if (qh MERGING) {
                            if (dist > 0) {
                                ...
                                maximize_(neighbor->maxoutside, dist);
                                                     ...
                                                     min_vertex, dist);  /* due to merge later */
                        }
                        trace2((qh
                                       ferr, 2057, "qh_findhorizon: point p%d is coplanar to horizon f%d, dist=%2.7g < qh MINvisible(%2.7g)\n",
                                               qh_pointid(point), neighbor->id, dist, qh
                                       MINvisible));
                    } else
...
QhullPoint p%d was above all facets.\n", qh_pointid(point));
    ...
    trace1((qh
                   ferr, 1041, "qh_findhorizon: %d horizon facets(good %d), %d visible(good %d), %d coplanar\n",
                           numhorizon, *goodhorizon, qh
...
void qh_partitionall(setT *vertices, pointT *points, int numpoints) {
    ...
    trace1((qh
                   ferr, 1042, "qh_partitionall: partition all points into outside sets\n"));
...
void qh_partitioncoplanar(pointT *point, facetT *facet, realT *dist) {
        ...
        if (!qh DELAUNAY && !qh KEEPinside) { /*  for 'd', bestdist skips upperDelaunay facets */
            if (qh KEEPnearinside) {
                if (bestdist < -qh NEARinside) {
                    ...
                    trace4((qh
                                   ferr, 4062, "qh_partitioncoplanar: point p%d is more than near-inside facet f%d dist %2.2g findbestnew %d\n",
                                           qh_pointid(point), bestfacet->id, bestdist, qh
                                   findbestnew));
                    return;
                }
            } else if (bestdist < -qh MAXcoplanar) {
                trace4((qh
                               ferr, 4063, "qh_partitioncoplanar: point p%d is inside facet f%d dist %2.2g findbestnew %d\n",
                                       qh_pointid(point), bestfacet->id, bestdist, qh
                               findbestnew));
                zinc_(Zcoplanarinside);
                return;
    ...
    if (bestdist > qh max_outside) {
        if (!dist && facet != bestfacet) {
            ...
            if (angle < 0) {
                ...
                zinc_(Zpartflip);
                trace2((qh
                               ferr, 2058, "qh_partitioncoplanar: repartition point p%d from f%d.  It is above flipped facet f%d dist %2.2g\n",
    ...
    trace4((qh
                   ferr, 4064, "qh_partitioncoplanar: point p%d is coplanar with facet f%d(or inside) dist %2.2g\n",
...
void qh_partitionpoint(pointT *point, facetT *facet) {
    ...
    if (isoutside) {
        ...
        qh num_outside++;
        trace4((qh
                       ferr, 4065, "qh_partitionpoint: point p%d is outside facet f%d new? %d (or narrowhull)\n",
        ...
        if ((qh KEEPcoplanar + qh KEEPnearinside) || bestdist > qh max_outside)
            ...
            trace4((qh
                           ferr, 4066, "qh_partitionpoint: point p%d is coplanar to facet f%d (dropped)\n",
        ...
        zinc_(Zpartinside);
        trace4((qh
                       ferr, 4067, "qh_partitionpoint: point p%d is inside all facets, closest to f%d dist %2.2g\n",
                               qh_pointid(point), bestfacet->id, bestdist));
        if (qh KEEPinside)
            qh_partitioncoplanar(point, bestfacet, &bestdist);
    }
...
void qh_partitionvisible(/*qh.visible_list*/ boolT allpoints, int *numoutside) {
    ...
    *numoutside = 0;
    ...
    trace1((qh
                   ferr, 1043, "qh_partitionvisible: partitioned %d points from outsidesets and %d points from coplanarsets\n", *numoutside, coplanar));
} /* partitionvisible */
...
void qh_precision(const char *reason) {
    ...
    if (qh ALLOWrestart && !qh PREmerge && !qh MERGEexact) {
        if (qh JOGGLEmax < REALmax / 2) {
            trace0((qh
                           ferr, 26, "qh_precision: qhull restart because of %s\n", reason));
...
Convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    ...
    qh_fprintf(fp, 9325, "  Number of distance tests for qhull: %d\n", zzval_(Zpartition) +
                                                                       zzval_(Zpartitionall) + zzval_(Znumvisibility) +
    ...
    if (nummerged) {
        qh_fprintf(fp, 9330, "  Number of distance tests for merging: %d\n", zzval_(Zbestdist) +
                                                                             zzval_(Zcentrumtests) +
                                                                             zzval_(Zdistconvex) + zzval_(Zdistcheck) +
                                                                             zzval_(Zdistzero));
