    ...
    if (insize >= 0 && insize <= qhmem.LASTsize) {
            ...
            *freelistp = *((void **) *freelistp);  /* replace freelist with next object */
            ...
            if (qhmem.IStracing >= 5)
                qh_fprintf(qhmem.ferr, 8141, "qh_mem %p n %8d alloc quick: %d bytes (tot %d cnt %d)\n", object, n,
                           outsize, qhmem.totshort, qhmem.cntshort + qhmem.cntquick - qhmem.freeshort);
#endif
            return (object);
        } else {
            qhmem.cntshort++;
        ...
        if (qhmem.IStracing >= 5)
            qh_fprintf(qhmem.ferr, 8057, "qh_mem %p n %8d alloc long: %d bytes (tot %d cnt %d)\n", object,
                       qhmem.cntlong + qhmem.freelong, outsize, qhmem.totlong, qhmem.cntlong - qhmem.freelong);
    }
    return (object);
} /* memalloc */
...
void qh_memcheck(void) {
    ...
    if (qhmem.ferr == 0 || qhmem.IStracing < 0 || qhmem.IStracing > 10 ||
        (((qhmem.ALIGNmask + 1) & qhmem.ALIGNmask) != 0)) {
        qh_fprintf_stderr(6244,
                          "qh_memcheck error: either qhmem is overwritten or qhmem is not initialized.  Call qh_meminit() or qh_new_qhull() before calling qh_mem routines.  ferr 0x%x IsTracing %d ALIGNmask 0x%x",
                          qhmem.ferr, qhmem.IStracing, qhmem.ALIGNmask);
        qh_exit(qhmem_ERRqhull);  /* can not use qh_errexit() */
    }
    if (qhmem.IStracing != 0)
        qh_fprintf(qhmem.ferr, 8143,
        ...
        *freelistp = object;
        ...
        if (qhmem.IStracing >= 5)
            qh_fprintf(qhmem.ferr, 8142, "qh_mem %p n %8d free short: %d bytes (tot %d cnt %d)\n", object, idx, outsize,
                       qhmem.totshort, qhmem.cntshort + qhmem.cntquick - qhmem.freeshort);
#endif
    } else {
        qhmem.freelong++;
        ...
        if (qhmem.IStracing >= 5)
            qh_fprintf(qhmem.ferr, 8058, "qh_mem %p n %8d free long: %d bytes (tot %d cnt %d)\n", object,
                       qhmem.cntlong + qhmem.freelong, insize, qhmem.totlong, qhmem.cntlong - qhmem.freelong);
        qh_free(object);
    }
} /* memfree */
