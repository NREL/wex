...
int qh_new_qhull(int dim, int numpoints, coordT *points, boolT ismalloc,
    ...
    if (numpoints == 0 && points == NULL) {
        trace1((qh
                       ferr, 1047, "qh_new_qhull: initialize Qhull\n"));
        return 0;
    }
    trace1((qh
                   ferr, 1044, "qh_new_qhull: build new Qhull for %d %d-d points with %s\n", numpoints, dim, qhull_cmd));
...
void qh_printfacetlist(facetT *facetlist, setT *facets, boolT printall) {
    ...
    FORALLfacet_(facetlist)qh_printafacet(qh ferr, qh_PRINTfacets, facet, printall);
    FOREACHfacet_(facets)qh_printafacet(qh ferr, qh_PRINTfacets, facet, printall);
...
to the center point are as follows:\n\n", qh DISTround);
    ...
    FORALLfacets {
        ...
        FOREACHvertex_(facet->vertices)qh_fprintf(fp, 9380, " p%d", qh_pointid(vertex->point));
        zinc_(Zdistio);
