...
void qh_distplane(pointT *point, facetT *facet, realT *dist) {
    ...
    switch (qh hull_dim) {
        ...
        case 5:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] + point[2] * normal[2] +
                    point[3] * normal[3] + point[4] * normal[4];
            break;
        case 6:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] + point[2] * normal[2] +
                    point[3] * normal[3] + point[4] * normal[4] + point[5] * normal[5];
            break;
        case 7:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] + point[2] * normal[2] +
                    point[3] * normal[3] + point[4] * normal[4] + point[5] * normal[5] + point[6] * normal[6];
            break;
        case 8:
            *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] + point[2] * normal[2] +
                    point[3] * normal[3] + point[4] * normal[4] + point[5] * normal[5] + point[6] * normal[6] +
...
facetT *qh_findbest(pointT *point, facetT *startfacet,
    ...
    while (facet) {
        trace4((qh
                       ferr, 4001, "qh_findbest: neighbors of f%d, bestdist %2.2g f%d\n",
...
*/
facetT *
qh_findbesthorizon(boolT ischeckmax, pointT *point, facetT *startfacet, boolT noupper, realT *bestdist, int *numpart) {
    ...
    while (True) {
        trace4((qh
                       ferr, 4002, "qh_findbesthorizon: neighbors of f%d bestdist %2.2g f%d ischeckmax? %d noupper? %d minsearch %2.2g searchdist %2.2g\n",
    ...
    if (!ischeckmax) {
        ...
        if (newbest) zinc_(Zparthorizon);
    }
    trace4((qh
                   ferr, 4003, "qh_findbesthorizon: newbest? %d bestfacet f%d bestdist %2.2g\n", newbest, getid_(
                           bestfacet), *bestdist));
    return bestfacet;
}  /* findbesthorizon */
...
facetT *qh_findbestnew(pointT *point, facetT *startfacet,
    ...
    *dist = bestdist;
    ...
    trace4((qh
                   ferr, 4004, "qh_findbestnew: bestfacet f%d bestdist %2.2g\n", getid_(bestfacet), *dist));
    qh IStracing = oldtrace;
    return bestfacet;
}  /* findbestnew */
...
void qh_backnormal(realT **rows, int numrow, int numcol, boolT sign,
    ...
    *normalp-- = (sign ? -1.0 : 1.0);
    ...
    if (zerocol != -1) {
        ...
        *nearzero = True;
        trace4((qh
                       ferr, 4005, "qh_backnormal: zero diagonal at column %d.\n", i));
...
pointT *qh_getcentrum(facetT *facet) {
    ...
    trace4((qh
                   ferr, 4007, "qh_getcentrum: for f%d, %d vertices dist= %2.2g\n",
        ...
        for (k = dim, colp = normal; k--; colp++) { /* k used below */
            ...
            if (!zerodiv)
                ...
                *maxp = temp;
                ...
                trace0((qh
                               ferr, 1, "qh_normalize: norm=%2.2g too small during p%d\n",
                                       norm, qh
                               furthest_id));
                return;
            }
    ...
    if (qh hull_dim > 4 || nearzero) {
        ...
        if (nearzero) {
            if (qh_orientoutside(facet)) {
                trace0((qh
                               ferr, 2, "qh_setfacetplane: flipped orientation after testing interior_point during p%d\n", qh
                               furthest_id));
...
void qh_sethyperplane_det(int dim, coordT **rows, coordT *point0,
    ...
    if (*nearzero) {
        zzinc_(Zminnorm);
        trace0((qh
                       ferr, 3, "qh_sethyperplane_det: degenerate norm during p%d.\n", qh
                       furthest_id));
        zzinc_(Znearlysingular);
    }
} /* sethyperplane_det */
...
void qh_sethyperplane_gauss(int dim, coordT **rows, pointT *point0,
    ...
    if (*nearzero) {
        zzinc_(Znearlysingular);
        trace0((qh
                       ferr, 4, "qh_sethyperplane_gauss: nearly singular or axis parallel hyperplane during p%d.\n", qh
                       furthest_id));
        ...
        if (nearzero2) {
            zzinc_(Znearlysingular);
            trace0((qh
                           ferr, 5, "qh_sethyperplane_gauss: singular or axis parallel hyperplane at normalization during p%d.\n", qh
                           furthest_id));
        }
    }
    if (nearzero2)
