...
void qh_premerge(vertexT *apex, realT maxcentrum, realT maxangle) {
    ...
    if (qh ZEROcentrum && qh_checkzero(!qh_ALL))
        return;
    trace2((qh
                   ferr, 2008, "qh_premerge: premerge centrum %2.2g angle %2.2g for apex v%d facetlist f%d\n",
                           maxcentrum, maxangle, apex->id, getid_(qh
                   newfacet_list)));
...
void qh_postmerge(const char *reason, realT maxcentrum, realT maxangle,
    ...
    trace2((qh
                   ferr, 2009, "qh_postmerge: postmerge.  test vneighbors? %d\n",
                           vneighbors));
    qh centrum_radius = maxcentrum;
    qh cos_max = maxangle;
    qh POSTmerging = True;
...
void qh_all_merges(boolT othermerge, boolT vneighbors) {
    ...
    trace2((qh
                   ferr, 2010, "qh_all_merges: starting to merge facets beginning from f%d\n",
                           getid_(qh
                   newfacet_list)));
    while (True) {
        wasmerge = False;
    ...
    trace1((qh
                   ferr, 1009, "qh_all_merges: merged %d coplanar facets %d concave facets and %d degen or redundant facets.\n",
...
setT *qh_basevertices(facetT *samecycle) {
    ...
    trace4((qh
                   ferr, 4019, "qh_basevertices: found %d vertices\n",
                           qh_setsize(vertices)));
    return vertices;
} /* basevertices */
...
boolT qh_checkzero(boolT testall) {
    ...
    if (testall)
        ...
        if (qh MERGEexact && qh ZEROall_ok) {
            trace2((qh
                           ferr, 2011, "qh_checkzero: skip convexity check until first pre-merge\n"));
            return True;
        }
    }
    FORALLfacet_(facetlist) {
    ...
    trace2((qh
                   ferr, 2012, "qh_checkzero: testall %d, facets are %s\n", testall,
                           (qh
                   MERGEexact && !testall) ?
    ...
    qh ZEROall_ok = False;
    trace2((qh
                   ferr, 2013, "qh_checkzero: facet f%d needs pre-merging\n",
                           facet->id));
    return False;

    LABELnonconvex:
    trace2((qh
                   ferr, 2014, "qh_checkzero: facet f%d and f%d are not clearly convex.  v%d dist %.2g\n",
...
void qh_copynonconvex(ridgeT *atridge) {
    ...
    FOREACHridge_(facet->ridges) {
            if (otherfacet == otherfacet_(ridge, facet) && ridge != atridge) {
                ...
                trace4((qh
                               ferr, 4020, "qh_copynonconvex: moved nonconvex flag from r%d to r%d\n",
                                       atridge->id, ridge->id));
                break;
            }
        }
...
void qh_degen_redundant_facet(facetT *facet) {
    ...
    trace4((qh
                   ferr, 4021, "qh_degen_redundant_facet: test facet f%d for degen/redundant\n",
                           facet->id));
    FOREACHneighbor_(facet) {
            qh vertex_visit++;
            ...
            if (!vertex) {
                ...
                trace2((qh
                               ferr, 2015, "qh_degen_redundant_facet: f%d is contained in f%d.  merge\n", facet->id, neighbor->id));
                return;
            }
        }
    if (qh_setsize(facet->neighbors) < qh hull_dim) {
        ...
        trace2((qh
                       ferr, 2016, "qh_degen_redundant_neighbors: f%d is degenerate.\n", facet->id));
    }
...
void qh_degen_redundant_neighbors(facetT *facet, facetT *delfacet) {
    ...
    trace4((qh
                   ferr, 4022, "qh_degen_redundant_neighbors: test neighbors of f%d with delfacet f%d\n",
                           facet->id, getid_(delfacet)));
    if ((size = qh_setsize(facet->neighbors)) < qh hull_dim) {
        qh_appendmergeset(facet, facet, MRGdegen, NULL);
        trace2((qh
                       ferr, 2017, "qh_degen_redundant_neighbors: f%d is degenerate with %d neighbors.\n", facet->id, size));
    }
    if (!delfacet)
        delfacet = facet;
    qh vertex_visit++;
    ...
    FOREACHneighbor_(delfacet) {
            ...
            if (!vertex) {
                ...
                trace2((qh
                               ferr, 2018, "qh_degen_redundant_neighbors: f%d is contained in f%d.  merge\n", neighbor->id, facet->id));
            }
        }
    FOREACHneighbor_(delfacet) {   /* redundant merges occur first */
            ...
            if ((size = qh_setsize(neighbor->neighbors)) < qh hull_dim) {
                ...
                trace2((qh
                               ferr, 2019, "qh_degen_redundant_neighbors: f%d is degenerate with %d neighbors.  Neighbor of f%d.\n", neighbor->id, size, facet->id));
            }
        }
...
vertexT *qh_find_newvertex(vertexT *oldvertex, setT *vertices, setT *ridges) {
    ...
    if (qh IStracing >= 4) {
        qh_fprintf(qh ferr, 8063, "qh_find_newvertex: find new vertex for v%d from ",
                   oldvertex->id);
        FOREACHvertex_(vertices)qh_fprintf(qh ferr, 8064, "v%d ", vertex->id);
        FOREACHridge_(ridges)qh_fprintf(qh ferr, 8065, "r%d ", ridge->id);
        qh_fprintf(qh ferr, 8066, "\n");
    }
#endif
    FOREACHvertex_(vertices)vertex->visitid = 0;
    FOREACHridge_(ridges) {
            FOREACHvertex_(ridge->vertices)vertex->visitid++;
        }
    ...
    if (!qh_setsize(vertices)) {
        trace4((qh
                       ferr, 4023, "qh_find_newvertex: vertices not in ridges for v%d\n",
                               oldvertex->id));
        return NULL;
    }
    ...
    if (vertex) {
        ...
        trace2((qh
                       ferr, 2020, "qh_find_newvertex: found v%d for old v%d from %d vertices and %d ridges.\n",
                               vertex->id, oldvertex->id, qh_setsize(vertices), qh_setsize(ridges)));
    } else {
        zinc_(Zfindfail);
        trace0((qh
                       ferr, 14, "qh_find_newvertex: no vertex for renaming v%d(all duplicated ridges) during p%d\n",
                               oldvertex->id, qh
                       furthest_id));
    }
...
facetT *qh_findbestneighbor(facetT *facet, realT *distp, realT *mindistp, realT *maxdistp) {
    ...
    *distp = REALmax;
    ...
    if (!bestfacet) {
        nonconvex = False;
        FOREACHneighbor_(facet)qh_findbest_test(testcentrum, facet, neighbor,
                                                &bestfacet, distp, mindistp, maxdistp);
    }
    if (!bestfacet) {
    ...
    trace3((qh
                   ferr, 3002, "qh_findbestneighbor: f%d is best neighbor for f%d testcentrum? %d nonconvex? %d dist %2.2g min %2.2g max %2.2g\n",
...
void qh_flippedmerges(facetT *facetlist, boolT *wasmerge) {
    ...
    trace4((qh
                   ferr, 4024, "qh_flippedmerges: begin\n"));
    FORALLfacet_(facetlist) {
    ...
    FOREACHmerge_(othermerges) {
            ...
            trace0((qh
                           ferr, 15, "qh_flippedmerges: merge flipped f%d into f%d dist %2.2g during p%d\n",
                                   facet1->id, neighbor->id, dist, qh
                           furthest_id));
    ...
    if (nummerge)
        *wasmerge = True;
    trace1((qh
                   ferr, 1010, "qh_flippedmerges: merged %d flipped facets into a good neighbor\n", nummerge));
} /* flippedmerges */
...
void qh_forcedmerges(boolT *wasmerge) {
    ...
    if (qh TRACEmerge - 1 == zzval_(Ztotmerge))
        qhmem.IStracing = qh IStracing = qh TRACElevel;
    trace4((qh
                   ferr, 4025, "qh_forcedmerges: begin\n"));
    othermerges = qh_settemppop(); /* was facet_mergeset */
    qh facet_mergeset = qh_settemp(qh TEMPsize);
    qh_settemppush(othermerges);
    FOREACHmerge_(othermerges) {
    ...
    if (nummerge)
        *wasmerge = True;
    trace1((qh
                   ferr, 1011, "qh_forcedmerges: merged %d facets and %d flipped facets across duplicated ridges\n",
                           nummerge, numflip));
} /* forcedmerges */
...
void qh_getmergeset(facetT *facetlist) {
    ...
    trace4((qh
                   ferr, 4026, "qh_getmergeset: started.\n"));
    qh visit_id++;
    FORALLfacet_(facetlist) {
            if (facet->tested)
                continue;
    ...
    if (qh ANGLEmerge)
        qsort(SETaddr_(qh
                               facet_mergeset, mergeT), (size_t) nummerges, sizeof(mergeT *), qh_compareangle);
    else
        qsort(SETaddr_(qh
                               facet_mergeset, mergeT), (size_t) nummerges, sizeof(mergeT *), qh_comparemerge);
    if (qh POSTmerging) {
    ...
    trace2((qh
                   ferr, 2021, "qh_getmergeset: %d merges found\n", nummerges));
} /* getmergeset */
...
void qh_getmergeset_initial(facetT *facetlist) {
    ...
    trace2((qh
                   ferr, 2022, "qh_getmergeset_initial: %d merges found\n", nummerges));
} /* getmergeset_initial */
...
void qh_makeridges(facetT *facet) {
    ...
    if (!facet->simplicial)
        return;
    trace4((qh
                   ferr, 4027, "qh_makeridges: make ridges for f%d\n", facet->id));
...
void qh_mark_dupridges(facetT *facetlist) {
    ...
    trace4((qh
                   ferr, 4028, "qh_mark_dupridges: identify duplicate ridges\n"));
    FORALLfacet_(facetlist) {
            if (facet->dupridge) {
                FOREACHneighbor_(facet) {
                        if (neighbor == qh_MERGEridge) {
    ...
    FOREACHmerge_(qh
                          facet_mergeset) {   /* restore the missing neighbors */
            if (merge->type == MRGridge) {
    ...
    trace1((qh
                   ferr, 1012, "qh_mark_dupridges: found %d duplicated ridges\n",
                           nummerge));
} /* mark_dupridges */
...
void qh_maydropneighbor(facetT *facet) {
    ...
    qh visit_id++;
    trace4((qh
                   ferr, 4029, "qh_maydropneighbor: test f%d for no ridges to a neighbor\n",
                           facet->id));
    FOREACHridge_(facet->ridges) {
    ...
    FOREACHneighbor_(facet) {
            if (neighbor->visitid != qh visit_id) {
                trace0((qh
                               ferr, 17, "qh_maydropneighbor: facets f%d and f%d are no longer neighbors during p%d\n",
                                       facet->id, neighbor->id, qh
                               furthest_id));
                zinc_(Zdropneighbor);
                ...
                if (qh_setsize(neighbor->neighbors) < qh hull_dim) {
                    ...
                    trace2((qh
                                   ferr, 2023, "qh_maydropneighbors: f%d is degenerate.\n", neighbor->id));
                }
            }
        }
    if (qh_setsize(facet->neighbors) < qh hull_dim) {
        ...
        trace2((qh
                       ferr, 2024, "qh_maydropneighbors: f%d is degenerate.\n", facet->id));
    }
} /* maydropneighbor */
...
int qh_merge_degenredundant(void) {
    ...
    while ((merge = (mergeT *) qh_setdellast(qh degen_mergeset))) {
        ...
        if (mergetype == MRGredundant) {
            ...
            trace2((qh
                           ferr, 2025, "qh_merge_degenredundant: facet f%d is contained in f%d, will merge\n",
                                   facet1->id, facet2->id));
            ...
            if (!(size = qh_setsize(facet1->neighbors))) {
                zinc_(Zdelfacetdup);
                trace2((qh
                               ferr, 2026, "qh_merge_degenredundant: facet f%d has no neighbors.  Deleted\n", facet1->id));
                ...
                FOREACHvertex_(facet1->vertices) {
                        ...
                        if (!SETfirst_(vertex->neighbors)) {
                            zinc_(Zdegenvertex);
                            trace2((qh
                                           ferr, 2027, "qh_merge_degenredundant: deleted v%d because f%d has no neighbors\n",
                                                   vertex->id, facet1->id));
                            vertex->deleted = True;
                ...
                trace2((qh
                               ferr, 2028, "qh_merge_degenredundant: facet f%d has %d neighbors, merge into f%d dist %2.2g\n",
...
void qh_merge_nonconvex(facetT *facet1, facetT *facet2, mergeType mergetype) {
    ...
    if (qh TRACEmerge - 1 == zzval_(Ztotmerge))
        qhmem.IStracing = qh IStracing = qh TRACElevel;
    trace3((qh
                   ferr, 3003, "qh_merge_nonconvex: merge #%d for f%d and f%d type %d\n",
    ...
    if (dist < dist2) {
        ...
        trace2((qh
                       ferr, 2029, "qh_merge_nonconvex: avoid merging old facet f%d dist %2.2g.  Use f%d dist %2.2g instead\n",
...
void qh_mergecycle(facetT *samecycle, facetT *newfacet) {
    ...
    if (qh TRACEmerge == zzval_(Ztotmerge))
        qhmem.IStracing = qh IStracing = qh TRACElevel;
    trace2((qh
                   ferr, 2030, "qh_mergecycle: merge #%d for facets from cycle f%d into coplanar horizon f%d\n",
                           zzval_(Ztotmerge), samecycle->id, newfacet->id));
    if (newfacet == qh tracefacet) {
        tracerestore = qh IStracing;
        qh IStracing = 4;
        qh_fprintf(qh ferr, 8068,
    ...
    if (qh IStracing >= 4) {
        ...
        FORALLsame_cycle_(samecycle)qh_fprintf(qh ferr, 8070, " f%d", same->id);
...
void qh_mergecycle_all(facetT *facetlist, boolT *wasmerge) {
    ...
    trace2((qh
                   ferr, 2031, "qh_mergecycle_all: begin\n"));
    ...
    if (cycles)
        *wasmerge = True;
    trace1((qh
                   ferr, 1013, "qh_mergecycle_all: merged %d same cycles or facets into coplanar horizons\n", cycles));
} /* mergecycle_all */
...
void qh_mergecycle_facets(facetT *samecycle, facetT *newfacet) {
    ...
    trace4((qh
                   ferr, 4030, "qh_mergecycle_facets: make newfacet new and samecycle deleted\n"));
    ...
    trace3((qh
                   ferr, 3004, "qh_mergecycle_facets: merged facets from cycle f%d into f%d\n",
...
void qh_mergecycle_neighbors(facetT *samecycle, facetT *newfacet) {
    ...
    trace4((qh
                   ferr, 4031, "qh_mergecycle_neighbors: delete shared neighbors from newfacet\n"));
    ...
    trace4((qh
                   ferr, 4032, "qh_mergecycle_neighbors: update neighbors\n"));
    FORALLsame_cycle_(samecycle) {
    ...
    trace2((qh
                   ferr, 2032, "qh_mergecycle_neighbors: deleted %d neighbors and added %d\n",
...
void qh_mergecycle_ridges(facetT *samecycle, facetT *newfacet) {
    ...
    trace4((qh
                   ferr, 4033, "qh_mergecycle_ridges: delete shared ridges from newfacet\n"));
    ...
    trace4((qh
                   ferr, 4034, "qh_mergecycle_ridges: add ridges to newfacet\n"));
    FORALLsame_cycle_(samecycle) {
    ...
    trace2((qh
                   ferr, 2033, "qh_mergecycle_ridges: found %d old ridges and %d new ones\n",
                           numold, numnew));
} /* mergecycle_ridges */
...
void qh_mergecycle_vneighbors(facetT *samecycle, facetT *newfacet) {
    ...
    trace4((qh
                   ferr, 4035, "qh_mergecycle_vneighbors: update vertex neighbors for newfacet\n"));
    ...
    FOREACHvertex_(vertices) {
            ...
            if (!SETsecond_(vertex->neighbors)) {
                zinc_(Zcyclevertex);
                trace2((qh
                               ferr, 2034, "qh_mergecycle_vneighbors: deleted v%d when merging cycle f%d into f%d\n",
    ...
    trace3((qh
                   ferr, 3005, "qh_mergecycle_vneighbors: merged vertices from cycle f%d into f%d\n",
                           samecycle->id, newfacet->id));
...
void qh_mergefacet2d(facetT *facet1, facetT *facet2) {
    ...
    trace4((qh
                   ferr, 4036, "qh_mergefacet2d: merged v%d and neighbor f%d of f%d into f%d\n",
...
void qh_mergeneighbors(facetT *facet1, facetT *facet2) {
    ...
    trace4((qh
                   ferr, 4037, "qh_mergeneighbors: merge neighbors of f%d and f%d\n",
                           facet1->id, facet2->id));
    qh visit_id++;
...
void qh_mergeridges(facetT *facet1, facetT *facet2) {
    ...
    trace4((qh
                   ferr, 4038, "qh_mergeridges: merge ridges of f%d and f%d\n",
                           facet1->id, facet2->id));
...
void qh_mergesimplex(facetT *facet1, facetT *facet2, boolT mergeapex) {
    ...
    if (mergeapex) {
        ...
        apex = vertex;
        trace4((qh
                       ferr, 4039, "qh_mergesimplex: merge apex v%d of f%d into facet f%d\n",
    ...
    trace4((qh
                   ferr, 4040, "qh_mergesimplex: update vertex neighbors of f%d\n",
                           facet1->id));
    FOREACHvertex_(facet1->vertices) {
    ...
    trace4((qh
                   ferr, 4041, "qh_mergesimplex: merge ridges and neighbors of f%d into f%d\n",
                           facet1->id, facet2->id));
    qh visit_id++;
    ...
    trace3((qh
                   ferr, 3006, "qh_mergesimplex: merged simplex f%d apex v%d into facet f%d\n",
...
void qh_mergevertex_del(vertexT *vertex, facetT *facet1, facetT *facet2) {
    ...
    zinc_(Zmergevertex);
    trace2((qh
                   ferr, 2035, "qh_mergevertex_del: deleted v%d when merging f%d into f%d\n",
...
void qh_mergevertex_neighbors(facetT *facet1, facetT *facet2) {
    ...
    trace4((qh
                   ferr, 4042, "qh_mergevertex_neighbors: merge vertex neighbors of f%d and f%d\n",
                           facet1->id, facet2->id));
    if (qh tracevertex) {
...
setT *qh_neighbor_intersections(vertexT *vertex) {
    ...
    trace3((qh
                   ferr, 3007, "qh_neighbor_intersections: %d vertices in neighbor intersection of v%d\n",
...
boolT qh_reducevertices(void) {
    ...
    trace1((qh
                   ferr, 1014, "qh_reducevertices: renamed %d shared vertices and %d redundant vertices. Degen? %d\n",
...
vertexT *qh_redundant_vertex(vertexT *vertex) {
    ...
    trace3((qh
                   ferr, 3008, "qh_redundant_vertex: check if v%d can be renamed\n", vertex->id));
...
boolT qh_remove_extravertices(facetT *facet) {
    ...
    trace4((qh
                   ferr, 4043, "qh_remove_extravertices: test f%d for extra vertices\n",
                           facet->id));
    ...
    FOREACHvertex_(facet->vertices) {
            if (!vertex->seen) {
                ...
                if (!qh_setsize(vertex->neighbors)) {
                    ...
                    zinc_(Zremvertexdel);
                    trace2((qh
                                   ferr, 2036, "qh_remove_extravertices: v%d deleted because it's lost all ridges\n", vertex->id));
                } else trace3((qh
                                      ferr, 3009, "qh_remove_extravertices: v%d removed from f%d because it's lost all ridges\n", vertex->id, facet->id));
                vertexp--; /*repeat*/
            }
        }
    return foundrem;
...
vertexT *qh_rename_sharedvertex(vertexT *vertex, facetT *facet) {
    ...
    trace2((qh
                   ferr, 2037, "qh_rename_sharedvertex: p%d(v%d) is shared by f%d(%d ridges) and f%d\n",
...
void qh_renameridgevertex(ridgeT *ridge, vertexT *oldvertex, vertexT *newvertex) {
    ...
    FOREACHvertex_(ridge->vertices) {
            if (vertex == newvertex) {
                ...
                trace2((qh
                               ferr, 2038, "qh_renameridgevertex: ridge r%d deleted.  It contained both v%d and v%d\n",
    ...
    if (abs(oldnth - nth) % 2) {
        trace3((qh
                       ferr, 3010, "qh_renameridgevertex: swapped the top and bottom of ridge r%d\n",
                               ridge->id));
        temp = ridge->top;
...
boolT qh_test_appendmerge(facetT *facet, facetT *neighbor) {
    ...
    if ((!qh MERGEexact || qh POSTmerging) && qh cos_max < REALmax / 2) {
        ...
        if (angle > qh cos_max) {
            ...
            trace2((qh
                           ferr, 2039, "qh_test_appendmerge: coplanar angle %4.4g between f%d and f%d\n",
    ...
    if (isconcave) {
        ...
        trace0((qh
                       ferr, 18, "qh_test_appendmerge: concave f%d to f%d dist %4.4g and reverse dist %4.4g angle %4.4g during p%d\n",
                               facet->id, neighbor->id, dist, dist2, angle, qh
                       furthest_id));
    } else /* iscoplanar */ {
        zinc_(Zcoplanarcentrum);
        qh_appendmergeset(facet, neighbor, MRGcoplanar, &angle);
        trace2((qh
                       ferr, 2040, "qh_test_appendmerge: coplanar f%d to f%d dist %4.4g, reverse dist %4.4g angle %4.4g\n",
...
boolT qh_test_vneighbors(void /* qh.newfacet_list */) {
    ...
    trace1((qh
                   ferr, 1015, "qh_test_vneighbors: testing vertex neighbors for convexity\n"));
    if (!qh VERTEXneighbors)
        qh_vertexneighbors();
    ...
    trace1((qh
                   ferr, 1016, "qh_test_vneighbors: found %d non-convex, vertex neighbors\n",
                           nummerges));
    return (nummerges > 0);
} /* test_vneighbors */
    ...
    if (facet2 == qh tracefacet || (qh tracevertex && qh tracevertex->newlist)) {
        ...
        if (facet2 != qh tracefacet)
            qh_errprint("TRACE", qh tracefacet,
                        ...
                        SETfirstt_(qh
                                           tracevertex->neighbors, facetT) : NULL,
                        NULL, qh tracevertex);
    }
    if (qh tracevertex) {
...
setT *qh_vertexridges(vertexT *vertex) {
    ...
    if (qh PRINTstatistics || qh IStracing) {
        ...
        trace3((qh
                       ferr, 3011, "qh_vertexridges: found %d ridges for v%d\n",
                               size, vertex->id));
    }
    return ridges;
} /* vertexridges */
