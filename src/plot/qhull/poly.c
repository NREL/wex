...
void qh_appendfacet(facetT *facet) {
    ...
    qh num_facets++;
    trace4((qh
                   ferr, 4044, "qh_appendfacet: append f%d to facet_list\n", facet->id));
} /* appendfacet */
...
void qh_appendvertex(vertexT *vertex) {
    ...
    qh num_vertices++;
    trace4((qh
                   ferr, 4045, "qh_appendvertex: append v%d to vertex_list\n", vertex->id));
} /* appendvertex */
...
void qh_attachnewfacets(void /* qh.visible_list, newfacet_list */) {
    ...
    qh NEWfacets = True;
    trace3((qh
                   ferr, 3012, "qh_attachnewfacets: delete interior ridges\n"));
    qh visit_id++;
    ...
    trace1((qh
                   ferr, 1017, "qh_attachnewfacets: attach horizon facets to new facets\n"));
    FORALLnew_facets {
...
boolT qh_checkflipped(facetT *facet, realT *distp, boolT allerror) {
    ...
    if ((allerror && dist > -qh DISTround) || (!allerror && dist >= 0.0)) {
        ...
        trace0((qh
                       ferr, 19, "qh_checkflipped: facet f%d is flipped, distance= %6.12g during p%d\n",
                               facet->id, dist, qh
                       furthest_id));
...
void qh_delfacet(facetT *facet) {
    ...
    trace4((qh
                   ferr, 4046, "qh_delfacet: delete f%d\n", facet->id));
    ...
    if (!facet->tricoplanar || facet->keepcentrum) {
        qh_memfree_(facet->normal, qh
                normal_size, freelistp);
        if (qh CENTERtype == qh_ASvoronoi) {   /* braces for macro calls */
            qh_memfree_(facet->center, qh
                    center_size, freelistp);
        } else /* AScentrum */ {
            qh_memfree_(facet->center, qh
                    normal_size, freelistp);
        }
...
void qh_deletevisible(void /*qh.visible_list*/) {
    ...
    trace1((qh
                   ferr, 1018, "qh_deletevisible: delete %d visible facets and %d vertices\n",
                           qh
                   num_visible, numdel));
    ...
    FOREACHvertex_(qh
                           del_vertices)qh_delvertex(vertex);
...
setT *qh_facetintersect(facetT *facetA, facetT *facetB,
    ...
    trace4((qh
                   ferr, 4047, "qh_facetintersect: f%d skip %d matches f%d skip %d\n",
...
void qh_makenewplanes(void /* newfacet_list */) {
    ...
    if (qh JOGGLEmax < REALmax / 2) minimize_(qh
                                                      min_vertex, -wwval_(Wnewvertexmax));
} /* makenewplanes */
    ...
    FOREACHridge_(visible->ridges) {
            ...
            if (neighbor->visible) {
                ...
                if (qh ONLYgood) {
                    ...
                    trace4((qh
                                   ferr, 4048, "qh_makenew_nonsimplicial: created facet f%d from v%d and r%d of horizon f%d\n",
...
facetT *qh_makenew_simplicial(facetT *visible, vertexT *apex, int *numnew) {
    ...
    FOREACHneighbor_(visible) {
            if (!neighbor->seen && !neighbor->visible) {
                ...
                trace4((qh
                               ferr, 4049, "qh_makenew_simplicial: create facet f%d top %d from v%d and horizon f%d skip %d top %d and visible f%d skip %d, flip? %d\n",
...
void qh_matchneighbor(facetT *newfacet, int newskip, int hashsize, int *hashcount) {
    ...
    trace4((qh
                   ferr, 4050, "qh_matchneighbor: newfacet f%d skip %d hash %d hashcount %d\n",
    ...
    for (scan = hash; (facet = SETelemt_(qh
        ...
        if (qh_matchvertices(1, newfacet->vertices, newskip, facet->vertices, &skip, &same)) {
            ...
            if (ismatch && !matchfacet) {
                ...
                (*hashcount)--;
                trace4((qh
                               ferr, 4051, "qh_matchneighbor: f%d skip %d matched with new f%d skip %d\n",
                                       facet->id, skip, newfacet->id, newskip));
                return;
            }
            if (!qh PREmerge && !qh MERGEexact) {
                qh_precision("a ridge with more than two neighbors");
                qh_fprintf(qh ferr, 6107,
            ...
            trace4((qh
                           ferr, 4052, "qh_matchneighbor: new f%d skip %d duplicates ridge for f%d skip %d matching f%d ismatch %d at hash %d\n",
    ...
    if (!newfound)
        SETelem_(qh
                         hash_table, scan) = newfacet;  /* same as qh_addhash */
    (*hashcount)++;
    trace4((qh
                   ferr, 4053, "qh_matchneighbor: no match for f%d skip %d at hash %d\n",
...
void qh_matchnewfacets(void /* qh.newfacet_list */) {
    ...
    trace1((qh
                   ferr, 1019, "qh_matchnewfacets: match neighbors for new facets.\n"));
    FORALLnew_facets {
        numnew++;
        {  /* inline qh_setzero(newfacet->neighbors, 1, qh hull_dim); */
            neighbors = newfacet->neighbors;
            neighbors->e[neighbors->maxsize].i = dim + 1; /*may be overwritten*/
            memset((char *) SETelemaddr_(neighbors, 1,
                           void), 0, dim * SETelemsize);
        }
    }
...
boolT qh_matchvertices(int firstindex, setT *verticesA, int skipA,
    ...
    *same = !((skipA & 0x1) ^ (*skipB & 0x1)); /* result is 0 or 1 */
    trace4((qh
                   ferr, 4054, "qh_matchvertices: matched by skip %d(v%d) and skip %d(v%d) same? %d\n",
...
facetT *qh_newfacet(void) {
    ...
    facet->newfacet = True;
    trace4((qh
                   ferr, 4055, "qh_newfacet: created facet f%d\n", facet->id));
    return (facet);
} /* newfacet */
...
qhull warning: more than 2^32 ridges.  Qhull results are OK.  Since the ridge ID wraps around to 0, two ridges may have the same identifier.\n");
    ...
    trace4((qh
                   ferr, 4056, "qh_newridge: created ridge r%d\n", ridge->id));
    return (ridge);
} /* newridge */
...
void qh_removefacet(facetT *facet) {
    ...
    qh num_facets--;
    trace4((qh
                   ferr, 4057, "qh_removefacet: remove f%d from facet_list\n", facet->id));
} /* removefacet */
...
void qh_removevertex(vertexT *vertex) {
    ...
    qh num_vertices--;
    trace4((qh
                   ferr, 4058, "qh_removevertex: remove v%d from vertex_list\n", vertex->id));
} /* removevertex */
...
void qh_updatevertices(void /*qh.newvertex_list, newfacet_list, visible_list*/) {
    ...
    trace3((qh
                   ferr, 3013, "qh_updatevertices: delete interior vertices and update vertex->neighbors\n"));
    if (qh VERTEXneighbors) {
        FORALLvertex_(qh
                              newvertex_list) {
        ...
        FORALLnew_facets {
            FOREACHvertex_(newfacet->vertices)qh_setappend(&vertex->neighbors, newfacet);
        }
        FORALLvisible_facets {
            FOREACHvertex_(visible->vertices) {
                    if (!vertex->newlist && !vertex->deleted) {
                        ...
                        if (neighbor)
                            ...
                            trace2((qh
                                           ferr, 2041, "qh_updatevertices: delete vertex p%d(v%d) in f%d\n",
        ...
        FORALLvisible_facets {
            FOREACHvertex_(visible->vertices) {
                    if (!vertex->newlist && !vertex->deleted) {
                        ...
                        qh_setappend(&qh del_vertices, vertex);
                        trace2((qh
                                       ferr, 2042, "qh_updatevertices: delete vertex p%d(v%d) in f%d\n",
                                               qh_pointid(vertex->point), vertex->id, visible->id));
                    }
                }
        }
