...
public:
    TextMessageDialog(const wxString &text, const wxString &title, wxWindow *parent, const wxSize &size, long buttons)
        ...
        wxTextCtrl *txtctrl = new wxTextCtrl(this, -1, text,
                                             ...
                                             wxTE_MULTILINE | wxTE_DONTWRAP | wxTE_READONLY | wxBORDER_NONE);
...
int wxShowTextMessageDialog(const wxString &text, const wxString &title, wxWindow *parent, const wxSize &size,
                            long buttons) {
    wxSize sz(size);
    if (sz == wxDefaultSize)
        sz.Set(600, 400);
