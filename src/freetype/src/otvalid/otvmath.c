...
otv_MathItalicsCorrectionInfo_validate(FT_Bytes table,
    ...
    otv_Coverage_validate(table + Coverage, otvalid, (FT_Int)
    cnt );
...
otv_MathVariants_validate(FT_Bytes table,
    ...
    if (VCoverage)
        otv_Coverage_validate(table + VCoverage, otvalid, (FT_Int)
    vcnt );

    OTV_SIZE_CHECK(HCoverage);
    if (HCoverage)
        otv_Coverage_validate(table + HCoverage, otvalid, (FT_Int)
    hcnt );
