...
sbit_table_size = 0;
face->
sbit_table_type = TT_SBIT_TABLE_TYPE_NONE;
face->
sbit_num_strikes = 0;
...
if ( !error )
face->
sbit_table_type = TT_SBIT_TABLE_TYPE_CBLC;
else
{
...
if ( !error )
face->
sbit_table_type = TT_SBIT_TABLE_TYPE_EBLC;
}
...
if ( !error )
face->
sbit_table_type = TT_SBIT_TABLE_TYPE_SBIX;
}
if ( error )
goto
Exit;
...
face->
sbit_table_size = 8 + count * 4;
if (
FT_FRAME_EXTRACT( face
->sbit_table_size, face->sbit_table ))
goto
Exit;
...
metrics->
ascender = (FT_Char) strike[16] * 64;  /* hori.ascender  */
metrics->
descender = (FT_Char) strike[17] * 64;  /* hori.descender */
...
{
metrics->
ascender = max_before_bl * 64;
metrics->
descender = min_after_bl * 64;
}
else
{
metrics->
ascender = metrics->y_ppem * 64;
metrics->
descender = 0;
}
}
...
metrics->x_ppem, metrics->y_ppem ));
metrics->
height = metrics->y_ppem * 64;
metrics->
descender = metrics->ascender - metrics->height;
}
...
{
map->
pixel_mode = new_map.pixel_mode;
map->
pitch = new_map.pitch;
map->
num_grays = new_map.num_grays;

ft_glyphslot_set_bitmap( face
->root.glyph, new_map.buffer );
face->root.glyph->internal->flags |=
FT_GLYPH_OWN_BITMAP;
}
}
