...
//	HRESULT hret = m_webBrowser->get_Document(&pDisp);
    ...
    if (disp.Ok()) {
        ...
        if (doc.Ok())
            doc->put_charset((BSTR)(
        const wchar_t *) charset.wc_str(wxConvUTF8));
...
public:
    ...
    *) { return E_NOTIMPL; }
    ...
    __RPC_FAR *) { return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE
    SetSize(ULARGE_INTEGER ) {return E_NOTIMPL;}
    HRESULT STDMETHODCALLTYPE
    CopyTo(IStream
    __RPC_FAR *, ULARGE_INTEGER ,
    ULARGE_INTEGER __RPC_FAR
    *,
    ULARGE_INTEGER __RPC_FAR
    *) { return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE
    Commit(DWORD ) {return E_NOTIMPL;}
    HRESULT STDMETHODCALLTYPE
    ...
    HRESULT STDMETHODCALLTYPE
    Stat(STATSTG
    __RPC_FAR *, DWORD ) { return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE
    Clone(IStream
    __RPC_FAR *__RPC_FAR
    *) { return E_NOTIMPL; }
...
void wxIEHtmlWin::LoadUrl(const wxString &url) {
    ...
    HRESULT hret = 0;
    hret = m_webBrowser->Navigate((BSTR)(
    const wchar_t *) url.wc_str(wxConvUTF8),
